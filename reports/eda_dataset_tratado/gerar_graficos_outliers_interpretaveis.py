...
def expandir_colunas_outlier(outliers: pd.DataFrame) -> pd.Series:
    # outlier_colunas vem como string separada por ";" - expande pra contar cada causa
    valores = []
...
def gerar_resumo(df: pd.DataFrame) -> None:
    ...
    # subplot 1: distribuição por tipo de outlier
    tipo_counts = (
    ...
    # subplot 2: quais colunas geraram os outliers
    ...
    # subplot 3: outliers por temporada
    if outliers.empty:
    ...
    # subplot 4: texto com resumo numérico
...
def gerar_distribuicoes(df: pd.DataFrame) -> None:
    # boxplot normal vs outlier pra cada métrica fastf1
    metricas = [
        "fastf1_avg_lap_time",
