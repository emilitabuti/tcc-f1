...
|---|---|---|---|---|
| `grid_position` | Numérica | Posição de largada (pit lane → 21) | Ergast + correção etapa 07 | Pronto |
...
| `recent_form_3` | Numérica | Média ponderada últimas 3 corridas (pesos 3,2,1) | finish_position histórico + shift(1) | **Pendente FE** |
...
| `driver_experience` | Numérica | Total de corridas na carreira até aquela data | Ergast acumulado + shift(1) | **Pendente FE** |
| `driver_wins_total` | Numérica | Total de vitórias na carreira até aquela data | Ergast acumulado + shift(1) | **Pendente FE** |
...
| Feature | Tipo | Descrição | Fonte | Status |
|---|---|---|---|---|
| `circuit_type` | Binária (0/1) | 0 = Permanente, 1 = Urbano/Temporário | `circuitos_manual.csv` | Pronto |
...
| Feature | Tipo | Descrição | Fonte | Status |
|---|---|---|---|---|
| `weather_impact_factor` | Numérica [0,1] | Score: (humidity_norm + 2×rain + (1−air_temp_norm)) / 4 | FastF1 weather | Pronto |

### Categoria: Eventos de pista

| Feature | Tipo | Descrição | Fonte | Status |
|---|---|---|---|---|
| `safety_car_flag` | Binária (0/1) | Corrida teve Safety Car ou VSC (histórico) | FastF1 TrackStatus 4/6/7 | Pronto |

---

## Features prontas na base FE-ready

Das 20 features finais, **12 já estão prontas** em
`data/processed/dataset_feature_engineering_ready_2018_2025.csv`:

- `grid_position`, `qualifying_position`, `grid_penalty`
- `circuit_type`, `track_complexity`, `altitude_m`
- `tire_compound_start`, `avg_pit_stops_circuit`
- `season_factor`
- `weather_impact_factor`
- `safety_car_flag`

**8 features ainda precisam ser calculadas** em `feature_engineering.py`:

- `recent_form_5`, `recent_form_3` (forma recente)
...
- `driver_experience`, `driver_wins_total`, `constructor_wins_total` (acumulados causais)
...
| `incident_rate_hist`, `incident_rate_hist_norm` | Componente de `track_complexity` (auditoria) |
...
FEATURES_FINAIS = [
    ...
    "recent_form_5", "recent_form_3",
    # Piloto (calcular na FE)
    "driver_coef_rapm", "driver_dnf_rate",
    "driver_experience", "driver_wins_total",
...
|---|---|---|
| `grid_position` | ≈ 0,71 | Advanced ML paper (Barra et al.) |
...
## Análise de correlação entre features (a executar antes da modelagem)

Pares com alta correlação esperada (r > 0,85 → considerar remover uma):
...
| `recent_form_3` vs `recent_form_5` | Moderada | Manter ambas (janelas distintas) |
| `grid_position` vs `qualifying_position` | Alta (exceto penalizados) | Avaliar correlação efetiva; manter `grid_position` |
