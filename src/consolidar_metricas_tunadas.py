...
def gerar_relatorio(df: pd.DataFrame, resumo: pd.DataFrame) -> None:
    ...
    ensembles = resumo[resumo["modelo"] != "ridge_baseline"].copy()
    mais_rapido = ensembles.sort_values("tempo_tuning_segundos").iloc[0]
    linhas = [
        ...
        "- Tempo de tuning lido dos arquivos de melhores parametros quando disponivel.",
        "",
        ...
        "- Esta consolidacao fecha a sexta-feira com metricas finais preliminares dos 4 modelos.",
        "",
        "Artefatos gerados:",
        f"- {OUTPUT_TABELA_4}",
        f"- {OUTPUT_RESUMO_4}",
