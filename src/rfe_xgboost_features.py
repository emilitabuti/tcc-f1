...
# pesos pra calcular o score final combinando as metricas
METRIC_WEIGHTS = {
    "mae_score": 0.35,
    "rmse_score": 0.20,
    "r2_score": 0.20,
...
# folds temporais: treina ate um ano e valida no seguinte
FOLDS_RFE = [
...
# lista de todas as features que vão entrar na seleção
FEATURES_CANDIDATAS = [
    "qualifying_position",
    "grid_penalty",
    "recent_form_5",
...
def load_data():
    ...
    # checa se alguma feature candidata sumiu do dataset
...
def build_model(seed=42):
    # XGBoost com os hiperparâmetros ajustados pra esse dataset
    return XGBRegressor(
...
def rank_features(model, features):
    # pega a importância por gain e por frequência de uso (weight)
...
def rank_features_temporal(x, y):
    ...
    # treina um modelo pra cada fold e coleta a importancia das features
    for fold in FOLDS_RFE:
        ...
        # normaliza o gain pra comparar entre os folds numa escala comum
    ...
    # une os rankings de todos os folds pela feature
    ranking = rankings[0]
    ...
    # média dos folds pra ter uma importância mais estável
...
def evaluate_subsets(x, y, ranking):
    ...
    # testa subconjuntos de 12 ate o total de features, adicionando uma por vez
    for k in range(12, len(ordered_features) + 1):
        ...
        for fold in FOLDS_RFE:
            ...
            # monta dataframe de predição pra calcular as métricas
        ...
        # media das metricas nos tres folds
        for metrica in METRICAS_AVALIACAO:
...
def normalizar_coluna(df, coluna, maior_melhor):
    # min-max pra escalar entre 0 e 1
    serie = df[coluna]
    minimo = serie.min()
    maximo = serie.max()
...
def adicionar_score_composto(subsets):
    # normaliza cada metrica e calcula score ponderado
...
def identificar_pareto(subsets):
    # metricas usadas pra dominancia de Pareto
    metricas = [
        ("mae_medio", False),
        ("rmse_medio", False),
        ("r2_medio", True),
    ...
    for idx, row in subsets.iterrows():
        ...
        for other_idx, other in subsets.iterrows():
            ...
            # se outro subconjunto domina em todas as métricas, esse é descartado
...
def main():
    ...
    # rankeia features, testa subconjuntos e acha os Pareto-ótimos
    ranking = rank_features_temporal(x, y)
    subsets = evaluate_subsets(x, y, ranking)
    pareto = identificar_pareto(subsets)

    # pega o melhor subconjunto pelo score composto
