...
INPUT_2025 = PROCESSED_DIR / "validacao_2025_clean.csv"
...
def main():
    ...
    # carrega X, y e o fold de validação 2025
    ...
    # X e y precisam ter o mesmo número de linhas
    if len(x) != len(y):
        erros.append(f"X e y com tamanhos diferentes: {len(x)} vs {len(y)}")

    # qualquer nulo nos dados de modelagem e problema
    if x.isna().sum().sum() > 0:
    ...
    if df_2025.isna().sum().sum() > 0:
        erros.append("validacao_2025_clean.csv contem valores nulos.")

    # verifica se as temporadas em y sao exatamente 2018-2025
    ...
    # o fold 2025 deve ter só 2025, nada mais
    if "season" not in df_2025.columns:
        erros.append("Coluna season ausente em validacao_2025_clean.csv.")
    elif set(df_2025["season"].unique()) != {2025}:
        erros.append(
            f"validacao_2025_clean.csv contem seasons inesperadas: {sorted(df_2025['season'].unique())}"
        )

    # confere se a quantidade de linhas 2025 bate entre y e o fold
    y_2025 = y[y["season"] == 2025].copy()
    if len(y_2025) != len(df_2025):
        erros.append(
            f"Quantidade de linhas 2025 divergente entre y e validacao_2025_clean: "
            f"{len(y_2025)} vs {len(df_2025)}"
        )

    # cross-check dos RaceIDs pra garantir que as corridas sao as mesmas
    ...
    # X so pode ter colunas numericas (sem strings, categorias etc.)
    features_numericas = [
    ...
    linhas = [
        ...
        f"- X shape: {x.shape}",
        f"- y shape: {y.shape}",
        f"- validacao_2025_clean shape: {df_2025.shape}",
