...
# candidatos ao fator de decay - vai de agressivo (0.50) ate quase sem decay (0.99)
DECAYS = [0.50, 0.65, 0.75, 0.85, 0.88, 0.90, 0.92, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99]
# treina ate um ano e valida no seguinte
FOLDS_OTIMIZACAO = [
    {"train_until": 2022, "valid_season": 2023},
    {"train_until": 2023, "valid_season": 2024},
]
# pesos do score composto - MAE tem mais peso porque é a métrica principal do TCC
METRIC_WEIGHTS = {
    "mae_score": 0.35,
    "rmse_score": 0.20,
    "r2_score": 0.20,
...
def criar_modelo_xgboost() -> XGBRegressor:
    # parametros fixos - aqui a gente nao tuna, so testa o efeito do decay
    return XGBRegressor(
...
def calcular_sample_weight(y_train: pd.DataFrame, valid_season: int, decay: float):
    # quanto mais antiga a corrida, menor o peso - decay^distancia
...
def normalizar_coluna(df: pd.DataFrame, coluna: str, maior_melhor: bool) -> pd.Series:
    # normaliza entre 0 e 1 - inverte quando menor é melhor (ex: MAE)
    serie = df[coluna]
    minimo = serie.min()
    maximo = serie.max()
...
def adicionar_score_composto(resumo: pd.DataFrame) -> pd.DataFrame:
    resumo = resumo.copy()
    # normaliza cada metrica e depois pondera conforme METRIC_WEIGHTS
...
def main():
    ...
    # roda cada combinação de decay x fold e acumula os resultados
    for decay in DECAYS:
    ...
    # agrega por decay (média dos dois folds) e calcula o score composto
    resumo = (
        df_resultados
    ...
    # grava o decay escolhido em txt para os outros scripts lerem
