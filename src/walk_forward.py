...
# cada fold treina ate um ano e valida no seguinte
FOLDS = [
...
def carregar_dados():
    # le os csvs de features e target
    x = pd.read_csv(INPUT_X)
    y = pd.read_csv(INPUT_Y)

    # checa se os tamanhos batem, senao algo deu errado no processamento
    if len(x) != len(y):
        raise RuntimeError(f"X e y com tamanhos diferentes: {len(x)} vs {len(y)}")

    # nao pode ter nulo - se tiver, para tudo
...
def carregar_decay_escolhido() -> float:
    # se o arquivo não existe, usa o padrão
    if not INPUT_DECAY.exists():
        return DECAY_PADRAO

    # procura a linha com o decay salvo da etapa de otimizacao
...
def criar_modelo_xgboost() -> XGBRegressor:
    # hiperparametros definidos na etapa anterior, sem tuning aqui
    return XGBRegressor(
...
def calcular_sample_weight(y_train: pd.DataFrame, valid_season: int, decay: float):
    # quanto mais antiga a temporada, menor o peso - decay exponencial
...
def rodar_fold(x, y, train_until: int, valid_season: int, decay: float):
    # separa treino e validacao pelo ano
    ...
    # pesos de treino com time-decay
    ...
    # monta o dataframe de predicoes com infos do fold
    ...
    # calcula as métricas e anota de qual fold vieram
...
def main():
    ...
    # roda cada fold e vai acumulando os resultados
    for fold in FOLDS:
