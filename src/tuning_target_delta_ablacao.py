...
# OBSOLETO / HISTORICO: target_delta foi rejeitado na versao final
...
# decay fixo nesse experimento historico
DECAY = 0.99
...
def sugerir_lgb(trial: optuna.Trial) -> dict:
    # mesmo espaco de busca do tuning oficial do LightGBM
...
def avaliar_tuning_delta(x: pd.DataFrame, y: pd.DataFrame, criar_modelo):
    # aqui o target e a diferenca entre posicao final e de quali, nao a posicao em si
    metricas = []
    for fold in FOLDS_TUNING:
        ...
        # reconstrói a posição final somando o delta previsto à posição de quali
...
def tunar_modelo(nome: str, x: pd.DataFrame, y: pd.DataFrame, sugerir, criar, trials: int):
    ...
    # reavalia os melhores params nos folds completos
    ...
    # salva os artefatos na pasta de ablacao
...
def main() -> None:
    # bloqueia execucao acidental - esse script nao faz parte do pipeline oficial
    raise SystemExit(
