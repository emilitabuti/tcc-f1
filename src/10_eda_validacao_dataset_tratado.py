...
# colunas que precisam existir pra o EDA fazer sentido
CRITICAL_COLUMNS = [
    "season",
    "round",
    "RaceID",
...
# limites esperados para cada coluna - fora disso e suspeito
RANGE_RULES = {
    "season": (2018, 2025),
    "round": (1, 30),
    "grid_position": (1, 24),
...
def ensure_dirs(output_dir: Path) -> dict[str, Path]:
    # cria as pastas de saída se ainda não existem
    dirs = {
        "root": output_dir,
...
def available_columns(df: pd.DataFrame, columns: list[str]) -> list[str]:
    # filtra so as colunas que realmente existem no dataframe
...
def run_ydata_profile(df: pd.DataFrame, output_dir: Path) -> dict[str, Any]:
    ...
    # tenta full primeiro, cai pra minimal se der erro
    attempts = [
        (
            "full",
            {
...
def run_sweetviz_reports(df: pd.DataFrame, output_dir: Path) -> dict[str, Any]:
    ...
    # compara treino 2018-2024 vs validacao 2025
    ...
    # ve se tem diferenca de distribuicao nas corridas com e sem safety car
...
def validate_dataset(df: pd.DataFrame) -> list[CheckResult]:
    ...
    # verifica se falta alguma coluna critica
    ...
    # cada piloto deve aparecer exatamente uma vez por corrida
    ...
    # checa se os valores numéricos estão dentro dos limites esperados
    ...
    # temporadas com menos de 100 registros provavelmente têm algum problema
    ...
    # uma corrida de F1 tem entre 10 e 24 pilotos
...
def run_great_expectations_core(df: pd.DataFrame, output_dir: Path) -> dict[str, Any]:
    # tenta rodar o GX Core nativo se estiver instalado; senão pula sem quebrar
    try:
...
def plot_temporal_summary(df: pd.DataFrame, path: Path) -> None:
    # evolução das métricas principais ao longo das temporadas
...
def dataset_summary(df: pd.DataFrame) -> dict[str, Any]:
    # estatísticas básicas da base pra incluir no relatório
...
def main() -> int:
    ...
    # se pediu pra pular relatórios pesados, verifica se já existe um salvo antes
    ...
    print(f"EDA concluido: {repo_relative(dirs['root'])}")
