"""
Extração FastF1 — qualifying e corridas, 2018–2024.

2018 e 2019 não têm dados no livetiming F1 (confirmado: 42 + 42 sessões testadas).
Período efetivo: 2020–2024 (~100 corridas, ~2.000 linhas de race, ~10.000 de qualifying).

Saídas:
  fastf1_qualifying_2018_2024.csv — tempos de qualifying por setor
    colunas: Driver, LapTime, Sector1Time, Sector2Time, Sector3Time, season, round

  fastf1_laps_2018_2024.csv — tempos de volta por setor + compostos de pneu (corrida)
    colunas: Driver, LapNumber, LapTime, Sector1Time, Sector2Time, Sector3Time,
             Compound, TyreLife, Stint, season, round
...
CKPT_FILE = os.path.join(DATA_RAW, "fastf1_checkpoint.json")
LOG_FILE  = os.path.join(DATA_RAW, "fastf1_extraction.log")
OUT_QUALI = os.path.join(DATA_RAW, "fastf1_qualifying_2018_2024.csv")
OUT_LAPS  = os.path.join(DATA_RAW, "fastf1_laps_2018_2024.csv")
...
def parse_qualifying(session, year: int, round_num: int) -> pd.DataFrame:
    """Tempos de qualifying por setor."""
...
def parse_race_laps(session, year: int, round_num: int) -> pd.DataFrame:
    """Tempos de volta por setor + compostos de pneu de cada volta da corrida."""
