...
def optimized_ensembles(rows: list[dict]) -> None:
    ...
    # Grade discreta restrita: pares completos e trios estratégicos. A grade
    # exaustiva com todos os modelos cresce rápido e não agrega comparação útil.
    combinacoes = []
...
def gerar_relatorio_completo(df: pd.DataFrame) -> None:
    valid = df.dropna(subset=["score_composto"])
    linhas = [
        "# Relatorio Completo de Estudos de Ablacao",
        "",
        "Este relatorio complementa a triagem inicial com retuning dos principais eixos do plano.",
        "",
